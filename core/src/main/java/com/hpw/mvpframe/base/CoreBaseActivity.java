...
public abstract class CoreBaseActivity<T extends CoreBasePresenter, E extends CoreBaseModel> extends SupportActivity {
    ...
    private void init(Bundle savedInstanceState) {
        ...
        setTheme(ThemeUtil.themeArr[SpUtil.getThemeIndex(this)][
                SpUtil.getNightModel(this) ? 1 : 0]);//设置主题
    ...
    public void reload() {
        ...
        overridePendingTransition(0, 0);//act 切换动画  放在mainifest的theme里更方便啊
        intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
        finish();
        overridePendingTransition(0, 0);  //start和finish后立即调用  这里位置有问题
        startActivity(intent);
    }

    @Override
    public void setContentView(int layoutResID) {  //如果用的statusbarutil  在基类里可以处理啊
        if (isOpen()) {
    ...
    /**
     * SwipeBackActivity中对SwipeBackLayout的初始化：
     * @return
     */
